...
\begin{frame}[fragile]{Performance: Restrict export list to \texttt{main}}
...
\begin{frame}[fragile]{Performance: Mark entries of Ray and Sphere as UNPACK and Strict}
...
\begin{frame}[fragile]{Performance: Use a pattern synonym to unpack Refl in Sphere}
...
\begin{frame}[fragile]{Performance: Change from \texttt{maximum} on a list to \texttt{max}}
...
\begin{frame}[fragile]{Performance:Convert \texttt{erand48} to pure Haskell}
...
\begin{frame}[fragile]{Performance: Remove mutability}
...
\begin{frame}[fragile]{Performance: Set \textbf{everything} in \texttt{smallpt} to be strict}
...
\begin{frame}[fragile]{Reduce to only useful strictnesses in \texttt{smallpt}}
...
\begin{frame}[fragile]{Performance: Strategic application of strictness in entire project}
...
\begin{frame}[fragile]{Performance: Remove \texttt{Maybe} from \texttt{intersect(s)}}
...
\begin{frame}[fragile]{Performance: Hand unroll the fold in intersects}
...
\begin{frame}[fragile]{Performance: Custom datatype for \texttt{intersects} parameter passing}
...
\begin{frame}[fragile]{Performance: Optimize file writing}
\includegraphics[height=0.6\textwidth]{perfdata-upto-4e141-gen.png}
\end{frame}


\begin{frame}[fragile]{Use \texttt{LLVM} backend ($2.04\times$)}
...
\begin{center}
\includegraphics[height=0.7\textwidth]{perfdata-upto-16c56-gen.png}
\end{center}
\note[item]{\textbf{D}}
\end{frame}

\begin{frame}[fragile]{Avoid CPU ieee754 slow paths  $(2.12\times)$}
\begin{minted}{diff}
 intersect :: Ray -> Sphere -> Double
 intersect (Ray o d) (Sphere r p _e _c _refl) =
   if det<0
...
\includegraphics[height=0.7\textwidth]{perfdata-upto-9ee06-gen.png}
\end{frame}
