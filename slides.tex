...
\note[item]{\textbf{S}}
\note[item]{Say that the core function is radiance}
\end{frame}
...
\end{minted}
\note[item]{\textbf{S}}
\note[item]{Say that it's recursive}
\end{frame}
...
\end{minted}
\note[item]{\textbf{S}}
\note[item]{and lots of arithmetic}
\end{frame}
...
\end{minted}
\note[item]{\textbf{S}}
\note[item]{Talk about how dense the source is to fit in 99 lines.}
\end{frame}
...
\note[item]{\textbf{S}}
\note[item]{show the same thing, this time in haskell}
...
\note[item]{\textbf{S}}
\note[item]{Draw attention to \texttt{Ray, Sphere} not having unpack}
\end{frame}
...
\end{minted}
\note[item]{\textbf{S}}
\note[item]{Draw attention the amount of numeric  stuff}
\note[item]{Draw attention the use of the list of spheres}
\end{frame}
...
This can save a bit in their evaluation cost but means we always pay it.
It is a trade off but here we know it will be a good one because we have almost perfect demand.}
\note[item]{Unpacking is different. It removes indirection from doing a memory lookup for components but means we have to copy everything into the data structure that it is unpacked into.}
\note[item]{We don't unpack ray because we'll be doing a lot of calculations on it and we want the Vec math to fuse and not get copied, when we use the Vecs they should be hot in cache anyway.}
\note[item]{We do inline Sphere because we statically create them and while they'll also be hot in cache there is no benefit to taking any extra cost at all.}
\end{frame}
...
\note[item]{\textbf{D}}
\note[item]{Until the recent addition of unboxed sums in GHC there was no way to unpack a type with multiple constructors.}
\note[item]{While unboxed sums exist now, their syntax is quite unpleasant.}
...
\note[item]{\textbf{D}}
\note[item]{finicky optimization. GHC does not evaluate at compile time, making optimizations like these necessary}
\note[item]{This function isn't used much so it doesn't really impact our performance but often building unnecessary data structures is detrimental.}
\end{frame}
...
\note[item]{HOWEVERm one primary optimization technique powering modern compilers is SSA.}
\note[item]{We almost start there as a functional language.}
...
\note[item]{Also if the work doesn't have to get done it still might be.}
...
\note[item]{After a little trial and error or reading core how to use bangs in your code will be intuitive.}
...
\note[item]{\textbf{D}}
\note[item]{Sadly 'intersects' remains one of our most costly functions even after that improvement so we'll focus on it a bit.}
\note[item]{A common optimization is loop unrolling.}
\note[item]{Many compilers do this for us, and there are special versions of it like Duff's Device.}
\note[item]{Sadly GHC is the one compiler I've used that doesn't.}
\note[item]{We can though implement every variety of it I know of by hand in Haskell, and this is one of the simplest.}
\note[item]{I believe we could produce a rule that did it for a given function but I've not actually explored that.}
...
\note[item]{\textbf{D}}
\note[item]{Still trying to focus on the performance of 'intersects' we can optimize how it passes data to itself and its caller.}
\note[item]{We want the data to be strict so the compiler knows the tuple's members will always be evaluated, but we want to avoid copying.}
\note[item]{A normal tuple lacks strictness information.}
\note[item]{An unboxed tuple sadly forces copying because of its unpacked nature. Sphere is quite large, and will be expensive to copy.}
\note[item]{What we need is a strict tuple, and we create one for just this usage.}
...
\note[item]{\textbf{D}}
\note[item]{ We're getting quite a bit faster now}
\note[item]{there's still some small pieces of low lying fruit.}
\note[item]{Strings are fairly inefficient and the conversion functions to them we use aren't particularly efficient.}
...
\note[item]{Know how the CPU (abstractly) executes.}
\end{frame}
